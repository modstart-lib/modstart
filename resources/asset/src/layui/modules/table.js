...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  String.prototype.contentWidth = function (body, font, isText) {
    var html = this;
    isText = isText || false
  ...
  Class.prototype.setColsWidth = function(){
    ...
    function autoCalcColumnWidth() {
      ...
      var th = that.elem.children('.layui-table-box').children('.layui-table-header').children('table').children('thead').children('tr').children('th'),
          fixTh = that.elem.children('.layui-table-box').children('.layui-table-fixed').children('.layui-table-header').children('table').children('thead').children('tr').children('th'),
          $tableBodyTr = that.elem.children('.layui-table-box').children('.layui-table-body').children('table').children('tbody').children('tr'),
          $totalTr = that.elem.children('.layui-table-total').find('tr');
      // console.log('autoCalcColumnWidth',that)
      // console.log('th',th)
      // console.log('fixTh',fixTh)
      // console.log('$tableBodyTr',$tableBodyTr)
      ...
      th.add(fixTh).on('dblclick', function (e) {
        ...
        var config = that.config.cols[colKey[1]][colKey[2]];
        computeColumnWidth(othis, othis.parents('.layui-table-fixed-r').length > 0 ? pLeft <= 10 : othis.width() - pLeft <= 10, config);
        setColumnWidth(othis, othis.parents('.layui-table-fixed-r').length > 0 ? pLeft <= 10 : othis.width() - pLeft <= 10, config);
      });
      // 初始化表格后，自动调整所有列宽
      var widthLeft = 0;
      ...
      th.add(fixTh).each(function (e, i) {
        ...
        if (config.withAuto === true) {
          computeColumnWidth( $(this), true, config);
          if (('widthAutoSize' in config) && ('widthAutoOld' in config)) {
            if(!widthLeftCheck[config.field]){
              widthLeftCheck[config.field] = true
      ...
      if (widthLeft > 0 && widthAutoJustCount > 0 && widthLeft>=widthAutoJustCount) {
        th.add(fixTh).each(function (e, i) {
          var colKey = $(this).attr('data-key').split('-');
          var config = that.config.cols[colKey[1]][colKey[2]];
          if (config.withAuto === true) {
      ...
      th.add(fixTh).each(function (e, i) {
        ...
        var config = that.config.cols[colKey[1]][colKey[2]];
        if (config.withAuto === true) {
          setColumnWidth( $(this), true, config);
        }
      });

      function setColumnWidth(othis, isHandle, config) {
      ...
      function computeColumnWidth(othis, isHandle, config) {
        var key = othis.data('key')
            , keyArray = key.split('-')
            , curKey = keyArray.length === 3 ? keyArray[1] + '-' + keyArray[2] : ''
        if (othis.attr('colspan') > 1 || othis.data('unresize')) {
        ...
        if (isHandle) {
          ...
          var maxWidth = othis.text().contentWidth(_BODY, othis.css('font')) + 21, font = othis.css('font');
          $tableBodyTr.children('td[data-key="' + key + '"]').each(function (index, elem) {
            var curWidth = 0
            if ($(this).children().children() && $(this).children().children().length > 0) {
              curWidth += $(this).children().html().contentWidth(_BODY, font)
            } else {
              curWidth = $(this).text().contentWidth(_BODY, font, true);
            }
          ...
          if ($totalTr.length > 0) {
            var curWidth = $totalTr.children('td[data-key="' + key + '"]').text().contentWidth(_BODY, font)
            if (maxWidth < curWidth) {
              maxWidth = curWidth
            }
          }
